...
namespace love_box2d
{
	...
	Vec2df Body::getWorldPoint( double x, double y ) {
		return body->GetWorldPoint(b2Vec2(x,y));
	}

	Vec2df Body::getWorldVector( double x, double y ) {
		return body->GetWorldVector(b2Vec2(x,y));
	}

	Vec2df Body::getLocalPoint( double x, double y ) {
		return body->GetLocalPoint(b2Vec2(x,y));
	}

	Vec2df Body::getLocalVector( double x, double y ) {
		return body->GetLocalPoint(b2Vec2(x,y));
	}

	Vec2df Body::getVelocityWorldPoint( double x, double y ) {
		return body->GetLinearVelocityFromWorldPoint(b2Vec2(x,y));
	}

	Vec2df Body::getVelocityLocalPoint( double x, double y ) {
		return body->GetLinearVelocityFromLocalPoint(b2Vec2(x,y));
	}
