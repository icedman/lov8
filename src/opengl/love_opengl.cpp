...
#include <v8.h>
extern v8::Handle<v8::ObjectTemplate> JSlove_openglCreateClass();
...
namespace love_opengl
{
	...
	bool module_open_v8(void * vm)
	{
		...
		(*love)->Set(v8::String::New("graphics"), JSlove_openglCreateClass());
		return true;
	}
	...
	DisplayModeList getModes()
	{
		...
		for(int i=0;modes[i];i++)
		{
			l.items[i].width = modes[i]->w;
			l.items[i].height = modes[i]->h;
			l.count = i;
		}
	...
	Rectf getScissor()
	{
		...
		r.x = scissor[0];
		r.y = getHeight() - (scissor[1] + scissor[3]);
		r.w = scissor[2];
		r.h = scissor[3];
	...
	LineStipple getLineStipple()
	{
		...
		glGetIntegerv(GL_LINE_STIPPLE_PATTERN, &l.pattern);
		glGetIntegerv(GL_LINE_STIPPLE_REPEAT, &l.factor);
	...
	**/
	...
	void polygon( int type, PointfList points )
	{
		...
		glBegin((type==love::DRAW_LINE) ? GL_LINE_LOOP : GL_POLYGON);

		for(int i = 0; i<points.length(); i++) {
			glVertex2f((GLfloat)points.at(i).x , (GLfloat)points.at(i).y);
		}
