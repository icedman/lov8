...
namespace love
{
	...
	bool Core::insmod(fptr_init init, fptr_quit quit, fptr_open open, fptr_open open_v8)
	{
		// Create the static module.
		pModule m(new StaticModule(init, quit, open, open_v8));
		return insmod(m);
	}
	...
	bool Core::openV8(void * vm)
	{
		for(int i = 0;i<(int)modules.size();i++)
		{
			if (modules[i]->module_open_v8)
				if(!modules[i]->module_open_v8(vm))
					return false;
		}
		return true;
	}
